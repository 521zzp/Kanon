import { USER_RE_POINT_TOTAL, USER_RE_POINT_LIST } from '../../config/url'
import { postModel, onanaly } from '../../utils/net'


export default {
  namespace: 'userRecordsPoint',
  state: {
  	total: 0, 
  	current: 1,
  	list: []
  ...
  effects: {
  	*getTotal ({ payload: obj }, { put, select }) {
  		...
  		const { total } = yield fetch(USER_RE_POINT_TOTAL, postModel({account, idCard})).then(onanaly)
  	...
  	*getList ({ payload: obj }, { put, select }) {
  		...
  		const list = yield fetch(USER_RE_POINT_LIST, postModel({current: obj, pageSize, account, idCard})).then(onanaly)
